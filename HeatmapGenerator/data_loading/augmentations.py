...
def sample_crop_best_center(image, input_size, random_number_generator, max_crop_noise, max_crop_size_noise,
    ...
    crop_noise_multiplier = np.zeros(2, dtype=float)
    ...
    borders = np.array((top, bottom, left, right), dtype=int)
    ...
    crop_noise = np.array((crop_noise[0], crop_noise[0], crop_noise[1], crop_noise[1]), dtype=int)
...
def sample_crop(image, input_size, borders, random_number_generator, max_crop_size_noise):
    ...
    size_noise = np.array(size_noise, dtype=int)
