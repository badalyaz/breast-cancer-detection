...
filePath = [pathToJPEGandDDSM2RAWfiles, caseName, '/', filename];
...
commandDecompression = ['"',cygwinLocation, '" -c "', pathToJPEGandDDSM2RAWfiles, './jpeg.exe -d -s ',filePath, '"'];
...
rawFileName          = [ pathToJPEGandDDSM2RAWfiles, caseName, '/', filename, '.1'];
columns              = num2str(columns);
rows                 = num2str(rows);
digitizer            = ['"' digitizer '"'];

commandConversion = ['"', cygwinLocation,'" -c "', pathToJPEGandDDSM2RAWfiles, './ddsmraw2pnm.exe ' rawFileName, ' ', columns, ' ',rows, ' ',digitizer '"'];
disp('Converting LJPEG1 -> RAW...');
dos(commandConversion);
%spl = strsplit(caseName,"/");
%caseName = strcat(spl(1),"\",spl(2));
